...
You can customize and integrate via our no-code solution: https://connect-in-style.wormhole.com/

### Via NPM (for React apps)
...
If you're not using React, you can still embed Connect on your website by using the hosted version. Simply copy and paste the following code into your HTML:

```html
<!-- Dependencies. Include in <head> -->
...
Note that the `wormhole-connect` div with your config has to be present _before_ the scripts are loaded.

## Configuration

Wormhole Connect is highly customizable via a simple `config` object passed in as a React prop:
