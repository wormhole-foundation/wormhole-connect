...
function Send(props: { valid: boolean }) {
  const { fromNetwork, toNetwork, token, amount, destGasPayment } = useSelector((state: RootState) => state.transfer);
  ...
  async function send() {
    ...
    await sendTransfer(token, `${amount}`, fromNetwork, sending.address, toNetwork, receiving.address, destGasPayment, '0');
    console.log('sent');
  }
  return (
    <Button
