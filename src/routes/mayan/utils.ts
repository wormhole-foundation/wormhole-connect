...
import { DEFAULT_DEADLINES, POSSIBLE_VAA_TYPES } from './consts';
...
export function getDefaultDeadline(chain: Chain): number {
  if (chain in DEFAULT_DEADLINES) return DEFAULT_DEADLINES[chain]! * 60;
  return 60 * 60;
}
...
export function txStatusToReceipt(txStatus: TransactionStatus): routes.Receipt {
  ...
  for (const vaaType of POSSIBLE_VAA_TYPES) {
