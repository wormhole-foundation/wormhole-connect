...
## More Networks / More Tokens

If you need to have an option to leave connect
carriying out some pre selected options you could
use the moreNetwoks / moreTokens options

### More Networks

allow you to specify a set of extra icons to be displayed as part of the
network selection modal, that can have an href link to send the user to
a different page/dApp/adroid or iOS link action 

|Property|description||
|:--|:--|:--:|
|`moreNetworks.href`| global href value that would be applied if the network does not have specified an href value|mandatory|
|`moreNetworks.target`| global href target value, could be any of the standard href target| optional, defaults to `_self`
|`moreNetworks.description` |brief description that should be displayed as tooltip when the user hover an more network icon, that you could use to inform the user the nature of this action| optional|
|`moreNetworks.networks[].icon` | url data enconded icon to display| mandatory|
|`moreNetworks.networks[].label` | | mandatory|
|`moreNetworks.networks[].name` | | optional, defaults to `moreNetworks.networks[].label.toLocaleLowerCase().split(' ').join('_')` |
|`moreNetworks.networks[].description` | local description value that could be used to override global one explaning the particular use case | optional, defaults to `moreNetworks.description`|
|`moreNetworks.networks[].target` | local href target value | optional, defaults to `moreNetworks.target`|
|`moreNetworks.networks[].showOpenInNewIcon`| allow you to disable top right open in new icon | optional, defaults to **true** if target is `_blank` or **false** if target is `_self`|
...
```json
{
  ...
  "moreNetworks": {
    ...
    "target": "_blank", //optional link target, by default this value is _self
    "description": "brief description that should be displayed as tooltip when the user over an more network icon",
    // network specific settings, will override global configs defined avobe
    "networks": [
      {
