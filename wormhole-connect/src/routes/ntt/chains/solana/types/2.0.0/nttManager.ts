...
import { BN, IdlAccounts, Program, web3 } from '@coral-xyz/anchor';
...
export class NttManager_2_0_0 {
  ...
  async receiveMessage(vaa: string, payer: string): Promise<string> {
    ...
    const ata = splToken.getAssociatedTokenAddressSync(mint, payerPublicKey);
    ...
    const transferIx = await this.program.methods
      ...
      .accountsStrict({
        common: {
          ...
          recipient: splToken.getAssociatedTokenAddressSync(
            mint,
            args.recipient,
          ),
          mint,
          tokenAuthority: this.tokenAuthorityAddress(),
          tokenProgram: args.config.tokenProgram,
          custody: await this.custodyAccountAddress(args.config),
        },
      })
      .instruction();

    const source = await this.custodyAccountAddress(args.config);
    const destination = splToken.getAssociatedTokenAddressSync(
      mint,
      args.recipient,
      true,
      args.config.tokenProgram,
    );
  ...
  async createRedeemInstruction(args: {
    ...
    return await this.program.methods
      ...
      .accounts({
        ...
        transceiver: this.registeredTransceiverAddress(this.program.programId),
        mint: await this.mintAccountAddress(args.config),
  ...
  async maybeAddExtraTransferHookAccounts(
    ...
    if (transferHook) {
      await splToken.addExtraAccountMetasForExecute(
        this.program.provider.connection,
        transferIx,
        ...
        destination,
        owner,
        // TODO: compute the amount that's passed into transfer.
