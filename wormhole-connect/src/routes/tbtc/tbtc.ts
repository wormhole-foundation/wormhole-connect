...
import { arrayify, getAddress, hexlify, parseUnits } from 'ethers/lib/utils.js';
...
export class TBTCRoute extends BaseRoute {
      ...
      if (isEvmChain(sendingChain)) {
        ...
        if (!!sourceAddress) {
          ...
          const estimateGas = await L2WormholeGateway.estimateGas.sendTbtc(
            amountNormalizeAmount,
            toChainId,
            chainContext.context.formatAddress(
              recipientAddress,
              recipientChain,
            ),
          ...
          const tbtcGateway = tryNativeToUint8Array(
            THRESHOLD_GATEWAYS[toChainId],
            toChainId,
          );
          const fromChainName = wh.toChainName(sendingChain);
          const recipient =
            `0x${uint8ArrayToHex(tbtcGateway)}` || recipientAddress;
  ...
  async redeem(
    ...
    if (isEvmChain(destChain)) {
      ...
      // With gateway
      if (!!THRESHOLD_GATEWAYS[wh.toChainId(destChain)]) {
