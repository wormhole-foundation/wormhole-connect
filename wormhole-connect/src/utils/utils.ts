...
export function toFixedNormalizedDecimals(
  amount: string,
  decimals: number,
  numDecimals?: number,
) {
  const normalized = toNormalizedDecimals(BigNumber.from(amount), decimals);
  return toFixedDecimals(normalized, numDecimals || 18);
}
